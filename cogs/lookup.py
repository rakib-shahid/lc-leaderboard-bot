...
class Lookup(commands.Cog):
    ...
    async def lookup(self, interaction: discord.Interaction, username: str):
        ...
        embed = discord.Embed(
            title=f"User Lookup - {username}", timestamp=datetime.datetime.now()
        )
        ...
        for url, header_title in zip(urls, header_titles):
            ...
            try:
                if response.status_code == 200:
                    ...
                    description += (
                        f":chart_with_upwards_trend: **Points**: {points:.2f}\n"
                    )
                    ...
                    description += (
                        f":earth_americas: **Global LeetCode Rank**: {global_ranking}\n"
                    )
                    ...
                    if leetcode_ac_response.status_code == 200:
                        ...
                        for i in range(min(5, json["count"])):
                            ...
                            try:  # catch odd error
                                difficulty = dbfuncs.get_points(
                                    problem_slug=json["submission"][i]["titleSlug"]
                                )
                            except:
                                difficulty = 0
                            difficulty_emoji = ""
                            ...
                            leetcode_ac.append(
                                f"{emoji}{difficulty_emoji} [{json['submission'][i]['title']}](https://leetcode.com/submissions/detail/{json['submission'][i]['id']}) - <t:{timestamp}:R>"
                            )
                    if leetcode_ac:
                        leetcode_ac_string = "\n".join(leetcode_ac)
